...
Which solution will meet this requirement with the LEAST operational overhead?
A. Configure a CloudWatch Logs subscription filter to stream the log group directly to Amazon OpenSearch Service (Amazon Elasticsearch Service).
...
| ------ | ----------------------------------- |
| A      | CloudFront doesn’t support DNS      |
| B      | ✅ Correct — TCP/UDP, global, HA     |
| C      | Fragile, wrong protocol, CloudFront |
| D      | ALB + CloudFront breaks DNS         |
...
Configure an EventBridge rule for the s3:ObjectCreated:Put event to invoke an AWS Lambda function for pattern-matching and to start the pipeline in Amazon SageMaker Pipelines.
...
Which solution will meet these requirements MOST cost-effectively?
A. Migrate the database to an Amazon RDS for MySQL DB instance. Create an AMI of the web application. Apply the AMI to a launch template. Create an Auto Scaling group with the launch template. Attach an Application Load Balancer to the Auto Scaling group.
